...
for filename, ftype, max_val in files_to_process:
    ...
    for cedula in CEDULAS:
        ...
        for r in new_entry.get("responses", []):
            # En Intra y Extra, las directas son: Siempre(1)=0_puntos, Nunca(5)=4_puntos
            # Para tener "muy_bajo" riesgo, queremos la mayoría de preguntas en 0 puntos (que es: Siempre=1)
            # Para "muy_alto" riesgo, queremos 4 puntos (Nunca=5).
            
            if tendencia == "muy_bajo":
                weights = [60, 30, 10, 0, 0] # Favor 1 y 2
            elif tendencia == "bajo":
                weights = [30, 40, 20, 10, 0] # Favor 2
            elif tendencia == "medio":
                weights = [10, 20, 40, 20, 10]
            elif tendencia == "alto":
                weights = [0, 10, 20, 40, 30] # Favor 4 y 5
            else: # muy_alto
                weights = [0, 0, 10, 30, 60] # Favor 5
